...
resource "aws_lb_listener" "http" {
  load_balancer_arn = module.ingress_alb.arn # arn : amazon resource name
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_lb_target_group" "expense" {
  ...
  protocol = "HTTP"
  vpc_id   = local.vpc_id
  # Here we give IP address as target group type ( For MicroServices )
  # For Monolithic Applications ( VM's ) we give instance as target group type
  target_type = "ip"
...
resource "aws_lb_listener_rule" "frontend" {
  ...
  condition {
    host_header {
      values = ["expense-${var.environment}.${var.zone_name}"] #expense-dev.ukom81s.onlie
    }
  }
}